...
import Add_new_note from '@/components/front-office-management/inquiry-tracking/Resolution_management/Add_new_note';
import Resolution_management_modal from '@/components/front-office-management/inquiry-tracking/Resolution_management/Resolution_management_modal';
import { ChevronDown, Filter, Search } from 'lucide-react'
import React, { useState } from 'react'
import { Link } from 'react-router-dom'

export default function Resolution_management() {
    const [] = useState([]);
    const [] = useState([]);
      ...
      <div className='p-4'>
        ...
        <Add_new_note/>
          <div className="nav mt-6 w-full rounded-md bg-white py-3 font-medium px-3 flex items-center gap-4">
              <Link to={'/inquiry-tracking'} className='pl-3 hover:text-red-500'>Inquiry tracking</Link>
              <Link to={'/resolution-management'} className='text-red-500'>Resolution management</Link>
              <Link to={'/feedback'} className='hover:text-red-500'>Resolution management</Link>
          </div>
          <div className="flex justify-between items-center">
              <h1 className="my-5 font-medium text-2xl">Resolution management</h1>
              <New_log/>
          </div>
                      ...
                      <tr key={data.id} className='relative'>
                        ...
                        <td className={'py-2 px-2 font-normal'}>
                            <div className="flex items-center gap-1 font-medium">
                              <p className="w-[30px] h-[30px] rounded-full bg-yellow-600"></p>{data.assigned}
                            </div>
                        </td>
                        <td className={'py-2 px-2 text-gray-500 font-normal'}>{data.status}</td>
                        <td className={'py-2 px-2 text-gray-500 font-normal'}>{data.date}</td>
                        <td>
                            <Resolution_management_modal data={data}/>
                        </td>
                      </tr>
                    )
                  })
